...
public final class NetworkFrameDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
        ...
        int serialNumber = connection.getSerialNumber();
        ByteBuf packet = S6EP3.deCrypt(frame, (short) serialNumber);
        if(in.getUnsignedByte(0) > 0xC2) {
            connection.plusAndGetSerialNumber();
        }
        //read all bytes so that an 'did not read anything but decoded a message' exception not throws
